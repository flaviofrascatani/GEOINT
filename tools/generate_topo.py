...
GRID_W, GRID_H = 32, 22          # 8 API requests per country
TARGET_LINES = 12
VIEW_W, VIEW_H = 400, 260
CHUNK = 100                       # Open-Meteo max locations per request
# Open-Meteo rate-limits shared GitHub runner IPs aggressively. Requests are
# strictly sequential, widely spaced and jittered; each run stops at its time
# budget and commits partial progress — re-runs (6-hourly schedule) resume
# from the missing countries until the whole set is done, however long it takes.
THROTTLE_S = float(os.environ.get("TOPO_THROTTLE_S", "10"))
TIME_BUDGET_S = float(os.environ.get("TOPO_TIME_BUDGET_MIN", "300")) * 60
UA = "Mozilla/5.0 (compatible; GEOINT-topo/1.0; +https://github.com)"

_last = [0.0]
def fetch_json(url, retries=8):
    for attempt in range(retries):
        wait = _last[0] + THROTTLE_S - time.time() + random.uniform(0, THROTTLE_S * .3)
        if wait > 0:
            time.sleep(wait)
        _last[0] = time.time()
        try:
            ...
            if e.code == 429:
                # Honour Retry-After if present, else long exponential backoff:
                # 2, 4, 8, 16… minutes. Patience beats the rate limiter.
                ra = e.headers.get("Retry-After") if e.headers else None
                pause = max(float(ra) if (ra and str(ra).isdigit()) else 0.0, 120.0 * (2 ** attempt))
                print(f"[topo] 429 rate-limited, sleeping {pause:.0f}s", file=sys.stderr)
                time.sleep(pause)
                continue
...
def main():
    ...
    # Strictly sequential: parallel workers defeat the throttle and trip
    # Open-Meteo's rate limiting (HTTP 429). Stop at the time budget so the
    # workflow can commit partial progress; the next scheduled run resumes.
    started = time.time()
    for idx, job in enumerate(jobs):
        if time.time() - started > TIME_BUDGET_S:
            print(f"[topo] time budget reached after {idx} countries — "
                  f"partial progress will be committed; the next run resumes automatically", file=sys.stderr)
            break
        cid, name, good, err = work(job)
        if good:
            ok += 1
            print(f"[topo] ok   {ok+fail}/{len(jobs)} {cid:<12} {name}", file=sys.stderr)
        else:
            fail += 1
            print(f"[topo] FAIL {ok+fail}/{len(jobs)} {cid:<12} {name}: {err}", file=sys.stderr)
