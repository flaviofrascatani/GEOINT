...
const INDEX_KEY='gx_region_index_v1';
...
const SIZES=[
  ...
  {km:100, label:'100 km', desc:'scala regionale · celle da ~4 km'}
];
...
// ============================================================================
//  Archivio locale
//  Ogni carta scaricata resta su QUESTO dispositivo, in localStorage: non
//  viene caricata da nessuna parte e nessun altro la vede. L'indice serve a
//  ritrovarle senza riscaricare nulla.
// ============================================================================
function readIndex(){
  try{
...
/** Nome leggibile: il sito piu' vicino al centro, altrimenti le coordinate. */
function labelFor(lat,lng,pts){
  ...
  return fmtLL(lat,'N','S')+' '+fmtLL(lng,'E','W');
}
function recordRun(entry){
  const a=readIndex().filter(e=>e.k!==entry.k);
  a.unshift(entry);
  writeIndex(a.slice(0,60));          // oltre non ha senso tenerne
}
...
function archiveBytes(){
  ...
  try{
    Object.keys(localStorage).forEach(k=>{
      if(k.indexOf(CACHE_KEY)===0||k===INDEX_KEY)
...
// ============================================================================
//  Terra e mare dai contorni veri
//  Ricavare la costa dal DEM a 50 km significa celle da 2 km: ne escono
//  poligoni squadrati. I contorni dei paesi che il sito ha gia' in memoria
//  hanno un dettaglio centinaia di volte superiore e non costano nulla.
// ============================================================================
function landPaths(bbox,V){
  ...
  const tolX=dLng/V*0.7, tolY=dLat/V*0.7;   // sotto il pixel non serve
  ...
  for(const id in cp){
    ...
    for(let q=0;q<polys.length;q++){
      ...
      // srotolamento e riallineamento alla finestra del riquadro
...
function buildSVG(bbox,km,elev,pts,centre){
  ...
  // Bastano una quindicina di punti di terra per definire i livelli. Con una
  // soglia percentuale, in uno stretto come Hormuz la terra restava sotto la
  // soglia e i livelli finivano calcolati su una griglia di zeri: da li' il
  // "rilievo assente" pur avendo quote fino a 541 m.
  const land=elev.filter(v=>v!=null&&v>0);
  const levels=pickLevels(land.length>=15?land:elev,TARGET_LINES);
  const interval=levels.length>1?(levels[1]-levels[0]):0;
  const hasSea=elev.some(v=>v!=null&&v<=0)&&land.length>0;   // c'e' acqua nel riquadro
  ...
  // --- terra e mare, dai contorni veri -------------------------------------
  const lps=landPaths(bbox,V);
  if(lps){
    // il mare copre tutto, poi la terra viene "ritagliata" sopra
    out+='<rect width="'+V+'" height="'+V+'" fill="rgba(74,138,162,.17)"/>';
    // ogni anello e' un path a se': unendoli in uno solo, versi opposti si
    // annullerebbero e alcune isole diventerebbero buchi
    lps.forEach(d=>{out+='<path d="'+d+'" fill="url(#rgbg)"/>'});
    out+='<defs><clipPath id="rgland">'+
      lps.map(d=>'<path d="'+d+'"/>').join('')+'</clipPath></defs>';
  }
  ...
  // --- isoipse (solo sulla terra, se la costa e' nota) ---------------------
  if(levels.length){
    ...
    // un riquadro tutto in mare non e' "terreno pianeggiante"
  ...
  // --- linea di costa: il contorno vettoriale, tracciato sopra le isoipse ---
  if(lps){
    lps.forEach(d=>{
      out+='<path d="'+d+'" fill="none" stroke="rgba(148,206,219,.8)" '+
...
.rg-arch-hd .rg-ic{font-size:8.5px;letter-spacing:.6px;padding:2px 7px}
...
.rg-sv .s{font-family:var(--mono,monospace);font-size:8.5px;
  color:var(--text-dim,#5a6d5e);letter-spacing:.3px;margin-top:1px}
...
.rg-arch-ft{font-family:var(--mono,monospace);font-size:8.5px;
  color:var(--text-dim,#5a6d5e);letter-spacing:.4px;line-height:1.6;
...
function build(){
  ...
  panel.innerHTML=
   ...
   '<div id="rg-arch" style="display:none">'+
     '<div class="rg-arch-hd"><span>Rilievi salvati su questo dispositivo</span>'+
       '<button class="rg-ic" id="rg-wipe" title="Svuota l\'archivio">svuota</button></div>'+
  ...
  panel.querySelector('#rg-arch-list').addEventListener('click',e=>{
    ...
    if(en){toggleSaved(false);run(en.lat,en.lng,en.km)}
  });
...
async function run(lat,lng,km){
  ...
  const valid0=res.elev.filter(v=>v!=null);
  recordRun({k:cacheKey(bbox,km),lat:lat,lng:lng,km:km,ts:Date.now(),
             mn:Math.min.apply(null,valid0),mx:Math.max.apply(null,valid0),
             np:pts.length,label:labelFor(lat,lng,pts)});
  refreshSaved();
...
function refreshSaved(){
  ...
  list.innerHTML=idx.length?idx.map(e=>
    '<div class="rg-sv" data-open="'+e.k+'">'+
      '<span class="km">'+e.km+' km</span>'+
      '<span class="b"><span class="t">'+esc(e.label||'')+'</span>'+
      '<span class="s">'+fmtLL(e.lat,'N','S')+' '+fmtLL(e.lng,'E','W')+
      ' · '+e.mn+'–'+e.mx+' m'+(e.np?' · '+e.np+' siti':'')+
      ' · '+whenTxt(e.ts)+'</span></span>'+
      '<span class="x" data-del="'+e.k+'" title="Elimina">✕</span>'+
    '</div>').join(''):
    '<div class="rg-arch-ft" style="border:0;margin:0;padding:0">'+
    'Nessun rilievo salvato. Quelli che scarichi compaiono qui.</div>';
  if(ft){
    const kb=Math.round(archiveBytes()/1024);
    ft.innerHTML='Tutto resta su questo dispositivo, in localStorage del browser: '+
      'niente viene caricato online e nessun altro lo vede.<br>'+
      'Occupazione: <b style="color:#8aad84;font-weight:400">'+kb+' KB</b>'+
