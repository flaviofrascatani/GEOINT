...
SYSTEM_PROMPT = (
    ...
    "TITLE RULES — the title is a map label, not a paragraph:\n"
    "  • Maximum 12 words / 90 characters. Shorter is better.\n"
    "  • Write it yourself as a compact headline. NEVER copy the numbered source "
    "line verbatim, and NEVER glue the article body onto the headline with a dash "
    "(no 'Headline — Reuters reports that on Tuesday...'). The body belongs in "
    "`description`, which is a separate field.\n"
    "  • No source name, no dateline, no trailing ellipsis.\n"
    "  • If the source headline is long or padded, rephrase it in fewer words while "
    "keeping the actor, the action and the place.\n\n"
    + SEVERITY_GUIDE +
...
- `title` must be your own compact headline: max 12 words / 90 characters, no source name, and never the source line copied with the article body appended after a dash.
...
def _shorten_title(title: str) -> str:
    """Collapse an over-long title down to a map-sized label.

    The model occasionally echoes the numbered source line, i.e.
    "Real headline — first 200 chars of the article body". That produced the
    200-character titles that overflow the event card. We drop the glued-on
    body, then trim on a word boundary. Purely deterministic: no API call, so
    it always runs even when the LLM rewrite pass is unavailable.
    """
    t = re.sub(r"\s+", " ", str(title or "")).strip()
    if not t:
        return ""
    # "Headline — body": keep the headline when it can stand on its own.
    parts = re.split(r"\s+[—–]\s+", t)
    if len(parts) > 1 and len(parts[0]) >= 25:
        t = parts[0].strip()
    if len(t) <= MAX_TITLE_CHARS:
        return t
    # Prefer cutting at a sentence end that still leaves a usable title...
    stop = t[:MAX_TITLE_CHARS].rfind(". ")
    if stop >= 40:
        return t[:stop + 1].strip()
    # ...otherwise at the last whole word before the limit.
...
REWRITE_SYSTEM = (
    "You rewrite news headlines into short map labels. For each numbered input "
    "you return one rewritten headline of at most 12 words that keeps the actor, "
    "the action and the place. No source names, no datelines, no ellipsis, no "
    "commentary. Reply with JSON only: {\"titles\": {\"0\": \"...\", \"1\": \"...\"}}"
)


def rewrite_long_titles(events: list) -> int:
    """Ask the LLM to rephrase titles that are still too long after trimming.

    One batched call for the whole run, so this costs a single extra request.
    Any failure leaves the deterministic `_shorten_title` output in place, which
    is already correct — just blunter.
    """
    targets = [
        (i, ev["title"]) for i, ev in enumerate(events)
        if len(ev.get("title", "")) > MAX_TITLE_CHARS or ev.get("title", "").endswith("…")
    ]
    if not targets:
        return 0
    listing = "\n".join(f"[{i}] {t}" for i, t in targets)
    try:
        raw = call_llm("Rewrite each headline:\n\n" + listing, REWRITE_SYSTEM)
        parsed = json.loads(raw) if raw.strip().startswith("{") else None
        if parsed is None:
            m = re.search(r"\{.*\}", raw, re.DOTALL)
            parsed = json.loads(m.group(0)) if m else {}
        titles = parsed.get("titles", {}) or {}
    except Exception as exc:  # noqa: BLE001 — degrade to the trimmed title
        print(f"[title] rewrite pass unavailable ({exc}); keeping trimmed titles",
              file=sys.stderr)
        return 0
    fixed = 0
    for idx, _old in targets:
        new = str(titles.get(str(idx), "")).strip().strip('"')
        if not new:
            continue
        new = _shorten_title(new)
        if new and not new.endswith("…"):
            events[idx]["title"] = new
            fixed += 1
    if fixed:
        print(f"[title] rewrote {fixed}/{len(targets)} over-long titles", file=sys.stderr)
    return fixed
...
def analyse(headlines: list, max_keep: int) -> list:
    ...
    for ev in events:
        ...
        title = _shorten_title(str(ev.get("title", "")).strip())
...
def main() -> int:
    ...
    # Any title that survived _shorten_title over the limit (or had to be cut
    # with an ellipsis) gets one batched rewrite request. Non-fatal by design.
    if os.environ.get("GEOINT_TITLE_REWRITE", "1") != "0":
        rewrite_long_titles(new_events)
